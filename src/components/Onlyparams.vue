...
export default {
  ...
  methods:{
    ...
    submituser(){
      this.$refs.ruleForm.validate(async valid=>{
        ...
        if(this.fromEdit){
          const {attr_id} = this.currentOnlyParams
          const {meta} = await editParams(id,attr_id,{attr_name, attr_sel:'only'})
          if(meta.status !== 200) return this.$message.error(meta.msg)
          this.$message.success(meta.msg)
        } else {
          const {meta} = await addParams(id,{attr_name, attr_sel:'only'})
          if(meta.status !== 201) return this.$message.error(meta.msg)
          this.$message.success(meta.msg)
        }
        this.dialogVisible=false
        this.$refs.ruleForm.resetFields()
        this.$parent.getOnlyParams()
      })
  ...
  computed:{
    fromEdit(){
      const { attr_id, attr_name } = this.currentOnlyParams
      return !!(attr_id || attr_name)
    }
  },
  watch:{
    currentOnlyParams(newValue){
